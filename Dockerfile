...
RUN python -c "import torch; from sam_audio import SAMAudio, SAMAudioProcessor; from sam_audio.ranking.imagebind import __imagebind_exists__; from core.audio_visual_encoder.transforms import AudioProcessor, PEAudioFrameTransform; assert torch.__version__.startswith('2.5.1'), torch.__version__; assert torch.version.cuda == '12.4', torch.version.cuda; assert __imagebind_exists__, 'imagebind runtime deps missing'; print(f'torch {torch.__version__} cuda {torch.version.cuda}'); print('sam_audio runtime imports ok')"
